...
class GenericGenerator(Visitor):
    ...
    def setBreakpoints(self):
        ...
        for b in breakpointList.findall('breakpoint'):
            breakpoints[b.get('state')] = 1
            #breakpoints.append(b.get('state'))
        return breakpoints
    ...
    def visit_StateChart(self, statechart):
        ...
        if self.debug_mode == 1:
            ...
            # create DEBUG state
            self.createDebugState(debugName, i)
        ...
        if self.debug_mode == 1:
            ...
            self.createDebugTransitions(debugName, statechart)
    ...
    def visit_EnterAction(self, enter_method):
        ...
        if self.debug_mode == 1:
            self.writer.addAssignment("timers", "[]")
        ...
        if self.debug_mode == 1:
            ...
            self.writer.addVSpace()
            self.chooseNextInputEvent(parent_node)
    ...
    def createDebugState(self, debugName, i):
        self.writer.addVSpace()    
        ...
        self.writer.addAssignment(
                index_expr,
                GLC.NewExpression(clazz, [str(i+1), GLC.String(debugName), GLC.SelfExpression()])
                )
        self.writer.add(
                        GLC.FunctionCall(
                            GLC.Property(
    ...
    def createDebugTransitions(self, debugName, statechart):
        ...
        for s in statechart.states:
            if not s.is_root:
                ...
                self.writer.add(
                    GLC.FunctionCall(
                        GLC.Property(toTransName,"setTrigger"),["pauseEvent"])
                                )
        ...
        #     toTransName = "%s_to_%s_breakpoint" % (b[1:], debugName[1:])
        #     self.writer.addComment(b + " to "+ debugName)      
        #     self.writer.addAssignment(
        #             GLC.LocalVariableDeclaration(toTransName),
        #             GLC.NewExpression(
        #                 "Transition",
    ...
    def chooseNextTransition(self, appended):
    ...
    def chooseNextInputEvent(self, parent_node):
        ...
        if parent_node.new_full_name in self.breakpoints:
            self.writer.add(GLC.FunctionCall(
                                GLC.Property(
                                GLC.MapIndexedExpression(
                                    GLC.SelfProperty("pauseTransitions"),
